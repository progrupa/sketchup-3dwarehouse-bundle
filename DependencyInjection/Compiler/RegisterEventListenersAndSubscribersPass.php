...
class RegisterEventListenersAndSubscribersPass implements CompilerPassInterface
{
    ...
    {
        $listeners = array();
        foreach ($container->findTaggedServiceIds('sketchup_3dwarehouse.serializer.event_listener') as $id => $tags) {
            if (!$container->getDefinition($id)->isPublic()) {
                throw new \RuntimeException(sprintf('The tag "sketchup_3dwarehouse.serializer.event_listener" of service "%s" requires the service to be public.', $id));
            }

            foreach ($tags as $attributes) {
                if ( ! isset($attributes['event'])) {
                    throw new \RuntimeException(sprintf('The tag "sketchup_3dwarehouse.serializer.event_listener" of service "%s" requires an attribute named "event".', $id));
                }
        ...
        foreach ($container->findTaggedServiceIds('sketchup_3dwarehouse.serializer.event_subscriber') as $id => $tags) {
        ...
        if ($listeners) {
            ...
            $container->getDefinition('sketchup_3dwarehouse.serializer.event_dispatcher')
